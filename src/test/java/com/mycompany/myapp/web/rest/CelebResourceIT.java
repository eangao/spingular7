...
import com.mycompany.myapp.domain.Celeb;
...
import com.mycompany.myapp.service.mapper.CelebMapper;
...
class CelebResourceIT {
    ...
    void createCeleb() throws Exception {
        ...
        restCelebMockMvc
            .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(celebDTO)))
    ...
    void putWithMissingIdPathParamCeleb() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCelebMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(celebDTO)))
    ...
    void patchWithMissingIdPathParamCeleb() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restCelebMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(celebDTO)))
            .andExpect(status().isMethodNotAllowed());
